...
bool bellmanFord(Digrafo g, Nodo n, vector<dist> &res)
{
    ...
    for (int j = 0; j < g._adylst.size(); j++)
    {
        for (int k = 0; k < g._adylst[j].size(); k++)
        {
            ...
            if (res[g._adylst[j][k].t] > aux)
            {
                ...
                if (!encontrado)
                {
                    segundoDelCiclo = g._adylst[j][k].t;
                    encontrado = true;
                }
            }
        }
    ...
    if (!encontrado) return true;
        ...
        ciclo.push_back(segundoDelCiclo);
        visitados[segundoDelCiclo] = 0;
        int actual = parents[segundoDelCiclo];
        int counter = 1;
        ...
        ciclo.push_back(actual);
        for (int i = visitados[actual]; i < ciclo.size(); i++)
        {
            cout << ciclo[i] << " ";
        }
